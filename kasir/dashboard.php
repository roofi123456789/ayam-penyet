...
if ($pay_col && $pay_col->num_rows > 0) {
    ...
    if ($r_pp)
        $stat_pending_pay = (int) $r_pp->fetch_assoc()['c'];
}
...
if ($filter_meja > 0)
    $where .= " AND p.nomor_meja = $filter_meja";
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .meja-badge-table .meja-label {
            font-size: 9px;
            font-weight: 600;
            opacity: 0.8;
        }

        .meja-badge-table .meja-num {
            font-size: 16px;
            font-weight: 800;
            line-height: 1;
        }
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <div class="sidebar-footer">
            <div class="admin-info">
                <div class="admin-avatar"><?= strtoupper(substr($_SESSION['user_nama'], 0, 1)) ?></div>
                <div>
                    <div class="admin-name"><?= htmlspecialchars($_SESSION['user_nama']) ?></div>
                    <div class="admin-role">Kasir</div>
                </div>
            </div>
    ...
    <div class="main-content">
        ...
        <div class="page-body">
            ...
            <div class="stats-grid">
                ...
                <?php if ($stat_pending_pay > 0): ?>
                    <div class="stat-card" style="border-left:4px solid #F59E0B;cursor:pointer"
                        onclick="window.location='konfirmasi_bayar.php'">
            ...
            <?php if ($stat_belum_bayar > 0): ?>
                <div id="unpaidWarn"
                    style="background:#FFF7ED;border:1px solid #FCD34D;border-radius:12px;padding:12px 18px;margin-bottom:16px;display:flex;align-items:center;gap:10px;font-size:13px;color:#92400E;font-weight:600">
                    <i class="fas fa-exclamation-triangle text-warning"></i>
                    ⚠️ <?= $stat_belum_bayar ?> pesanan selesai belum bayar — segera proses di kasir!
                </div>
            <?php else: ?>
                <div id="unpaidWarn"
                    style="display:none;background:#FFF7ED;border:1px solid #FCD34D;border-radius:12px;padding:12px 18px;margin-bottom:16px;align-items:center;gap:10px;font-size:13px;color:#92400E;font-weight:600">
                </div>
            <?php endif; ?>
            ...
            <div class="filter-bar">
                ...
                <a href="?status=pending" class="filter-pill <?= $filter_status === 'pending' ? 'active' : '' ?>">⏳
                    Pending</a>
                <a href="?status=diproses" class="filter-pill <?= $filter_status === 'diproses' ? 'active' : '' ?>">🔥
                    Diproses</a>
                <a href="?status=selesai" class="filter-pill <?= $filter_status === 'selesai' ? 'active' : '' ?>">✅
                    Selesai</a>
            </div>
                    ...
                    <?php foreach ($pesanan_list as $p): ?>
                        <div class="order-row">
                            ...
                            <div class="order-info">
                                <div class="order-kode">
                                    ...
                                    <?php if (($p['nama_pelanggan'] ?? '') && $p['nama_pelanggan'] !== 'Pelanggan'): ?>
                                        <span
                                            style="margin-left:6px;background:#EFF6FF;color:#2563EB;border-radius:50px;padding:2px 8px;font-size:11px;font-weight:700">
                            ...
                            <div class="d-flex flex-column align-items-end gap-1">
                                ...
                                <?php if ($sb === 'lunas'): ?>
                                    <span
                                        style="background:#ECFDF5;color:#16A34A;border-radius:50px;padding:2px 8px;font-size:10px;font-weight:700;white-space:nowrap">
                                        ✅ LUNAS <?= $mb ? '·' . strtoupper($mb) : '' ?>
                                    </span>
                                <?php else: ?>
                                    <span
                                        style="background:#FEF2F2;color:#DC2626;border-radius:50px;padding:2px 8px;font-size:10px;font-weight:700;white-space:nowrap">
                                        ⏳ Belum Bayar
                                    </span>
                                <?php endif; ?>
                            </div>
                            ...
                            <div class="d-flex flex-column gap-1">
                                ...
                                <?php if (($p['status_bayar'] ?? '') === 'lunas' || $p['status'] === 'selesai'): ?>
                                    <a href="struk.php?kode=<?= htmlspecialchars($p['kode_pesanan']) ?>" target="_blank"
                                        class="btn-detail"
                                        style="font-size:11px;background:#EFF6FF;color:#2563EB;border-color:#BFDBFE;text-align:center">
    ...
    <script>
        ...
        function lihatDetail(id) {
            ...
            fetch(`api_kasir.php?action=get_detail&id=${id}`)
                .then(r => r.json())
                .then(data => {
                    if (data.success) {
                        let html = `
                <div style="background:#F8F9FA;border-radius:10px;padding:14px;margin-bottom:16px">
                    ...
                    ${(() => {
                        const nm = (data.pesanan.nama_pelanggan || '').trim();
                        const nmDisplay = (nm && nm !== '0') ? nm : 'Pelanggan';
                        ...
                        <span style="font-size:13px;font-weight:700;color:#2563EB">👤 ${nmDisplay}</span>
                    </div>`;
                    })()}
                    <div style="display:flex;justify-content:space-between;margin-bottom:6px">
                ...
                <table style="width:100%;border-collapse:collapse">
                    ...
                    <tfoot>
                        ...
                        <a href="struk.php?kode=${data.pesanan.kode_pesanan}" target="_blank"
